...
def test_load_flat_dataset():
    ...
    data_flat = EvalDataset.from_json(str(json_dataset_path), label="label")
...
def test_load_split_dataset():
    ...
    data_split = EvalDataset.from_json(str(json_dataset_dict_path), label="label", split="train")
...
def test_load_csv_dataset():
    ...
    data_csv = EvalDataset.from_csv(str(csv_dataset_path), label="label")
...
def test_load_huggingface_dataset():
    data_hf = EvalDataset.from_huggingface("imdb", label="label", split="test")
...
def test_nonexistent_files():
    ...
    with pytest.raises(FileNotFoundError, match="File not found"):
        EvalDataset.from_csv("nonexistent.csv", label="label")

    # Test nonexistent JSON file
    with pytest.raises(FileNotFoundError, match="File not found"):
        EvalDataset.from_json("nonexistent.json", label="label")
...
def test_invalid_split():
    ...
    with pytest.raises(ValueError, match="Split 'testing' not found in JSON file"):
        EvalDataset.from_json(str(json_dataset_path), label="label", split="testing")
